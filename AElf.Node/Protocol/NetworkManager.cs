    ...
    public class NetworkManager : INetworkManager
    {
        ...
        public NetworkManager(ITxHub txHub, IPeerManager peerManager, IChainService chainService, ILogger logger, IBlockSynchronizer blockSynchronizer)
        {
            ...
            MessageHub.Instance.Subscribe<BlockAddedToSet>(inBlock =>
            {
                ...
                _logger?.Info($"Block produced, announcing {inBlock.Block.BlockHashToHex} to peers {string.Join("|", _peers)} " +
                              $"with {inBlock.Block.Body.TransactionsCount} txs. Block height {inBlock.Block.Header.Index}.");
            ...
            MessageHub.Instance.Subscribe<BlockExecuted>(inBlock =>
            {
                ...
                _logger?.Trace($"Block accepted, announcing {inBlock.Block.BlockHashToHex} to peers {string.Join("|", _peers)}. " +
        ...
        private void HandleBlockReception(AElfProtocolMsgType msgType, Message msg, Peer peer)
        {
            try
            {
                ...
                _logger?.Info($"Receive block of height {block.Index} - {block.BlockHashToHex} from peer {peer}.");
